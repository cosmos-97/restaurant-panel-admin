...
class CrudSpecial(CrudBase[Special, SpecialCreate, SpecialUpdate]):
    def get_multi(
        self,
        db: Session,
        *,
        ...
        try:
            column = getattr(Special, column)
        except Exception:
            raise HTTPException(status_code=404, detail="column name not found")

        return (
            db.query(Special)
            .filter(Special.is_archived == is_archived)
            .order_by(asc(column))
            .offset(skip)
            .limit(limit)
            .all()
        ), db.query(Special).filter(Special.is_archived == is_archived).count()
    ...
    def search(
        ...
        *,
        ...
        return (
            ...
            .filter(
                and_(
                    Special.__ts_vector__.op("@@")(func.plainto_tsquery(term)),
                    Special.is_archived == is_archived,
                ),
            )
            .order_by(asc(column))
            .offset(skip)
    ...
    def create_with_owner(
        self, db: Session, *, obj_in: SpecialCreate, user_id: int
    ) -> Special:
    ...
    def update(
        ...
        db: Session,
        *,
        db_obj: SpecialCreate,
        obj_in: Union[SpecialUpdate, Dict[str, Any]],
    ) -> Special:
